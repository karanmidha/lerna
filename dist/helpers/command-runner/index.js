...
function commandRunner(commandModule) {
  ...
  const cmd = commandModule.command.split(" ")[0]; // prime the pump so slow-as-molasses CI doesn't fail with delayed require()
  ...
  return cwd => {
    ...
    return (...args) => new Promise((resolve, reject) => {
      ...
      const context = {
        ...
        onRejected: result => {
          Object.assign(result, yargsMeta); // tests expect errors thrown to indicate failure,
      ...
      cli.fail((msg, err) => {
        ...
        const actual = err || new Error(msg); // backfill exitCode for test convenience
