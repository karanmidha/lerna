...
function createDependencyMatcher(dependencyType) {
  ...
  return (received, name, range, options) => {
    ...
    const version = pkg[dependencyType][name].replace(/[\\]/g, "/"); // we don't care about semver intersection, it's not always a semver range
...
function toHaveBinaryLinks(received, ...inputs) {
  const pkg = Package.lazy(received);
  const links = process.platform === "win32" ? inputs.reduce((acc, input) => [...acc, input, // cmd.exe
  [input, "cmd"].join("."), // powershell
  [input, "ps1"].join(".")], []) : inputs;
  const expectedName = `expected ${pkg.name}`;
  ...
  if (missing.length > 0 || superfluous.length > 0) {
    return {
      message: () => [expectedName, expectedAction, missing.length > 0 ? `missing: ${missing.join(", ")}` : "", superfluous.length > 0 ? `superfluous: ${superfluous.join(", ")}` : ""].filter(Boolean).join("\n"),
      pass: false
    };
  }
...
function toHaveExecutables(received, ...files) {
  ...
  const expectation = `${expectedFiles} ${expectedAction}`; // eslint-disable-next-line prefer-destructuring
  ...
  return {
    message: () => pass ? `${expectedFiles} not ${expectedAction}` : `${expectation} while ${failed.join(", ")} ${verb} found to be not executable.`,
    pass
  };
}