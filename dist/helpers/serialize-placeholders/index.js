...
const serializeWindowsPaths = require("@lerna-test/serialize-windows-paths"); // eslint-disable-next-line node/no-unpublished-require


const LERNA_VERSION = require("../../core/lerna/package.json").version;

const VERSION_REGEX = new RegExp(`^((?:.*?notice cli )|\\^?)v?${LERNA_VERSION}`, "g"); // TODO: maybe even less naïve regex?
...
module.exports = {
  test(thing) {
    ...
    return isString(thing) && (serializeWindowsPaths.test(thing) || serializeTempdir.test(thing) || VERSION_REGEX.test(thing) || // always remove redundant quotes from multiline strings
    /\n/.test(thing));
  },
